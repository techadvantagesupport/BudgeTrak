...
object EnglishStrings : AppStrings {
    ...
    override val settings = SettingsStrings(
        ...
        aiOcrAddReceiptFirst = "Add a receipt photo first",
        aiOcrHighlightFirst = "Long-press a photo to highlight it as the scan target",
        aiOcrReading = "Reading receipt…",
        aiOcrFailed = "Couldn't read the receipt — please enter manually",
        aiOcrHelpTitle = "AI Receipt Scan",
        aiOcrHelpBody = "Subscribers can tap the sparkle icon in the transaction dialog to auto-fill merchant, date, amount, and category from a receipt photo.\n\nPicking which photo to scan: long-press a thumbnail in the photo bar to highlight it with a blue outline \u2014 that highlighted photo becomes the AI's scan target. Long-press again to clear the highlight, or long-press a different thumbnail to move it. Supplemental photos (item close-ups, storefront, etc.) can sit un-highlighted in the other slots. If you tap the sparkle without highlighting a photo, a toast will remind you to long-press one first.\n\nReordering photos: after long-pressing a thumbnail (don't lift your finger), slide it left or right over the other photos and the row will reshuffle in real time. Release to commit the new order.\n\nDeleting a photo: tap the thumbnail to open the full-screen view, then use the delete button there.\n\nSYNC'd receipts: if a photo was added on another device and hasn't finished downloading here yet, you'll see a placeholder with a camera icon. You can reorder it just like a regular photo; tapping it shows a short toast noting the download is still in progress.",
        aiOcrPreselectBanner = "Read the help (?) section on preselected categories regarding receipt scanning behavior!",
        aiOcrPreselectHelpTitle = "Pre-selected categories & receipt scanning",
        aiOcrPreselectHelpBody = "How the AI's category split depends on what you've already selected in the dialog:\n\n\u2022 No categories selected \u2014 the AI is free to choose from your full category list and will pick whichever it thinks best fit the items on the receipt. This is the right mode for a fresh receipt where you haven't committed to any bucket yet.\n\n\u2022 Categories pre-selected \u2014 the AI is told to only consider those specific buckets. It will split the receipt's items across the ones you picked (and will skip a pre-selected category if nothing on the receipt plausibly fits it, rather than force-filling it).\n\nImportant: the AI does NOT change your category selection. Whatever boxes are checked when you tap the sparkle stay checked afterwards; the AI only fills in amounts. That means if you're editing an existing transaction where (say) only Other is selected, the AI will put the whole receipt total into Other \u2014 it won't add Groceries or Home Supplies even if it reads those items on the receipt.\n\nTo let the AI take a fresh look at categories on an existing transaction, deselect every category first, then tap the sparkle.",
    ...
    override val transactions = TransactionsStrings(
        ...
        importAiNetworkHint = "Connect to Wi-Fi or mobile data before importing for best categorization.",
        importAiBusy = "Smart categorizing transactions…",
    ...
    override val sync = SyncStrings(
        ...
        claimApproved = "You have been approved as the new administrator.",
        claimRejected = "Your request to assume administrator duties was rejected.",
