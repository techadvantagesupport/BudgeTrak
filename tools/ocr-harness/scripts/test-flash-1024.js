#!/usr/bin/env node
// Flash 2.5 with thinkingBudget=1024, T1+T5 combo prompt.
// Tests whether Flash@1024 is viable as multi-cat tier — same thinking budget
// as Pro@1024 but much cheaper input/output rates.
...
async function main() {
  ...
  console.log(`Flash 2.5 @ thinkingBudget=1024 on ${receipts.length} receipts × 2 runs = ${receipts.length*2} calls\n`);
  ...
  for (const label of receipts) {
    ...
    for (const run of [1, 2]) {
      process.stdout.write(`  T12_flash1024 run${run} `);
      try {
        const t0 = Date.now();
  ...
  const outFile = path.join(ROOT, "results", `flash-1024-${new Date().toISOString().replace(/[:.]/g,"-")}.json`);
  fs.writeFileSync(outFile, JSON.stringify({ run: "flash-1024", results }, null, 2));
