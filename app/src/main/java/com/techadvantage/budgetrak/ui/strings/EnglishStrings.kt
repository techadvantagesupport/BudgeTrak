...
object EnglishStrings : AppStrings {
    ...
    override val settings = SettingsStrings(
        ...
        aiCsvCategorizeHelpBody = "When enabled, BudgeTrak sends the merchant, amount, and date of newly-imported bank transactions to Google's Gemini AI to pick the best-matching category for each one.\n\nBudgeTrak already learns from your past transactions on-device; the AI only gets involved for unfamiliar merchants where the on-device matcher can't be confident. Connect to Wi-Fi or mobile data before importing for best results.\n\nYour data is encrypted in transit (HTTPS) and deleted by Google when the request completes — nothing is stored on Google's servers. The AI provider never sees your account, your balances, other transactions, or receipt photos.\n\nTurn this off at any time to fall back to BudgeTrak's on-device matcher only.",
