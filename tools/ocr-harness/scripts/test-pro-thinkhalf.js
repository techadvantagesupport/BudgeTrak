#!/usr/bin/env node
// Third variant for the combo prompt comparison: thinkingBudget = 3000 (half
// of the ~6000-token default observed in earlier Pro runs). Lets us see if a
// reduced thinking budget keeps most of the quality while cutting cost.
//
// Same prompt as T6_combo/T7_nothink (T1_thematic + T5_nomp).
...
// Two variants: min thinking (128 = Pro's floor) and half-budget (3000).
// Pro 2.5 doesn't accept thinkingBudget=0 ("Budget 0 is invalid") so 128 is
// the effective "no thinking" value.
const VARIANTS = [
  { id: "T7b_min",  budget: 128,  label: "minimum thinking (Pro floor=128)" },
  { id: "T8_half",  budget: 3000, label: "half thinking (~half of 6500 default)" },
];
const client = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

async function call({ imageBytes, mimeType, prompt, budget }) {
  let lastErr;
...
async function main() {
  ...
  console.log(`Pro combo T7b+T8 on ${receipts.length} receipts × ${VARIANTS.length} variants × 2 runs = ${receipts.length*VARIANTS.length*2} Pro calls\n`);
  ...
  for (const label of receipts) {
    ...
    for (const v of VARIANTS) {
      for (const run of [1, 2]) {
        process.stdout.write(`  ${v.id.padEnd(9)} run${run} `);
        try {
          const t0 = Date.now();
          const r = await call({ imageBytes: img, mimeType: mime, prompt, budget: v.budget });
  ...
  for (const v of VARIANTS) {
    ...
    console.log(`${v.id.padEnd(9)} ${v.label.padEnd(42)} m ${m}/${n}  d ${d}/${n}  a ${a}/${n}  sum ${sum}/${n}  cset ${cset}/${n}  cshr ${cshr}/${n}  inTok ${avgIn.toFixed(0)}  visOut ${avgVis.toFixed(0)}  hidden ${avgHidden.toFixed(0)}  ${avgMs.toFixed(0)}ms  ~\$${estCostPerCall.toFixed(4)}/call`);
  }

  const outFile = path.join(ROOT, "results", `pro-thinkhalf-${new Date().toISOString().replace(/[:.]/g,"-")}.json`);
  fs.writeFileSync(outFile, JSON.stringify({ run: "pro-thinkhalf", results }, null, 2));
