...
- Firebase Analytics (added 2026-04-22): per-event usage telemetry, same opt-out toggle. Events: `ocr_feedback` (fires on save of OCR-populated transactions — measures per-field user-correction rate), `health_beacon` (daily sync-user heartbeat, migrated from Crashlytics non-fatal). Helper: `data/telemetry/AnalyticsEvents.kt`. Setting renamed to "Send crash reports and anonymous usage data".
