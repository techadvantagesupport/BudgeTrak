#!/usr/bin/env node
// Combined T1+T5 test (thematic first-pass without merchant-priority) against
// the same prompt with thinking disabled (thinkingBudget=0). Ran together so
// we can see both the quality win and the cost impact of disabling thinking.
//
// Variants:
//   T6_combo:     T1_theme + T5_nomp, thinking default (~6k reasoning tokens/call)
//   T7_nothink:   same prompt, thinkingBudget=0 (no reasoning billing)
...
// Combined T1+T5: thematic first-pass, no MP
...
async function call({ imageBytes, mimeType, prompt, thinkingBudget }) {
  const config = {
    responseMimeType: "application/json",
    responseSchema: SCHEMA,
    temperature: 0,
  };
  // thinkingBudget = null → SDK default (dynamic). 0 = disabled.
...
const VARIANTS = [
  { id: "T6_combo",    thinkingBudget: null, label: "T1+T5 combo (thinking default)" },
  { id: "T7_nothink",  thinkingBudget: 0,    label: "T1+T5 combo (thinking DISABLED)" },
];
...
async function main() {
  ...
  console.log(`Pro combo + no-think on ${receipts.length} receipts × ${VARIANTS.length} variants × 2 runs = ${receipts.length*VARIANTS.length*2} Pro calls\n`);
  ...
  for (const label of receipts) {
    ...
    for (const v of VARIANTS) {
      for (const run of [1, 2]) {
        process.stdout.write(`  ${v.id.padEnd(11)} run${run} `);
        try {
          const t0 = Date.now();
          const r = await call({ imageBytes: img, mimeType: mime, prompt, thinkingBudget: v.thinkingBudget });
          ...
          results.push({ file: label.file, variant: v.id, run, grade, sumMatch: drift <= 0.05, drift, tokens: t, hidden, ms });
        } catch (e) {
  ...
  for (const v of VARIANTS) {
    ...
    console.log(`${v.id.padEnd(11)} ${v.label.padEnd(42)} m ${m}/${n}  d ${d}/${n}  a ${a}/${n}  sum ${sum}/${n}  cset ${cset}/${n}  cshr ${cshr}/${n}  inTok ${avgIn.toFixed(0)}  visOut ${avgVis.toFixed(0)}  hidden ${avgHidden.toFixed(0)}  ${avgMs.toFixed(0)}ms  ~\$${estCostPerCall.toFixed(4)}/call`);
  }

  const outFile = path.join(ROOT, "results", `pro-combo-${new Date().toISOString().replace(/[:.]/g,"-")}.json`);
  fs.writeFileSync(outFile, JSON.stringify({ run: "pro-combo", results }, null, 2));
